...
class ScarEngine:
    ...
    def create(self,lesson,context,severity=8,kind='decision_failure',impact=None):
        ...
        s=Scar(id='scar_'+datetime.now(timezone.utc).strftime('%Y%m%d')+'_'+uuid.uuid4().hex[:5],type=kind,severity=severity,lesson=lesson,principle='Use independent verification and a reversible test before repeating this class of decision.',context=context,cooldown_until=cooldown,impact=impact)
